...
//
function addHighScore(record, node,i,neededHighlight) {
    ...
    if(neededHighlight){
        document.getElementById("name"+i).setAttribute('style','fill:blue');
    }
...
//
function showHighScoreTable(table,position) {
    // Show the table
    ...
    for (var i = 0; i < 5; i++) {
        ...
        // from the for loop
        if (i >= table.length) break;
        var isCurrentPlayer = (i==position);
        // Add the record at the end of the text node
        addHighScore(table[i], node,i,isCurrentPlayer);
    }
}
